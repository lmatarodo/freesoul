...
def kanayama_control(lane_info):
...
def test_single_frame_visualization():
    ...
    steering_angle, calculated_speed = kanayama_control(lane_info)
    ...
    if is_jupyter_environment():
        ...
        cv2.putText(info_img, f"Left X: {lane_info.left_x:.1f}", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
        cv2.putText(info_img, f"Right X: {lane_info.right_x:.1f}", (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        cv2.putText(info_img, f"Objects: {len(bev_boxes)}", (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
...
def test_video_visualization(max_frames=30):
    ...
    while frame_count < max_frames:
        ...
        # 진행 상황 출력
        if frame_count % 10 == 0:
            print(f"Frame {frame_count}: Steering {steering_angle:.1f}°, Speed {calculated_speed:.1f} m/s")
